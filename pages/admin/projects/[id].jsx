    ...
    updateProject(id, formdata, token)
      .then((data) => {
        if (data.error) {
          ...
          (sel_ref.current.value = ""), (file_ref.current.value = "");
        }
      })
    ...
    <>
      <div className="bg-blue-200 h-screen">
        <div className="border-2 shadow-lg bg-white rounded-md mb-10 ms-7 lg:w-3/5 md:w-5/6 w-10/12 xl:p-10 md:p-5 p-2">
          <h1 className="font-bold lg:text-3xl lg:text-left text-center md:text-2xl">
            Update Projects
          </h1>
          {showError()}
          {showSuccess()}
            ...
            <div className="col-6">
              <form className="lg:w-full w-10/12 md:text-xl text-sm">
                ...
                <input
                  ...
                  name="project_title"
                  value={project_title}
                  className="border border-black p-1 md:text-lg rounded-md md:h-8 h-7 "
                ...
                <input
                  ...
                  id="image"
                  name="project_image"
                  className="border border-black p-1 md:text-lg rounded-md md:h-11 h-10"
                  onChange={handleChange("project_image")}
                  ref={file_ref}
                />
              </form>
              <button
                onClick={handleSubmit}
                className="border border-none bg-blue-600 rounded-md md:w-20 md:h-10 w-14 h-7 lg:mt-8 mt-3 md:text-lg text-sm"
              >
